...
const generatePosts = (count, titles, sentences, categories) => Array(count).fill({}).map(() => generatePost(titles, sentences, categories));
...
module.exports = {
  ...
  async run(args) {
    ...
    let content = JSON.stringify(generatePosts(postsCount, titles, sentences, categories));
